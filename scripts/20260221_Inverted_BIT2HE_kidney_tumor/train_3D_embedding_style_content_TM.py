...
def parse_cmdargs():
    parser = argparse.ArgumentParser(
        description = f'{today_str}_BIT2HE_tumor_kidney_3DFlow_Multiscale_Content'
    )
    ...
    parser.add_argument(
        '--debug-adjacent-pairs',
        action='store_true',
        help='Debug-only: instantiate AdjacentZPairDataset for domain A and save first pair'
    )
    ...
    add_batch_size_parser(parser, default = 2)
...
def get_transfer_preset(cmdargs):
    ...
    if cmdargs.base_model is not None:
        ...
        pretrain_root = (
         '/home/durrlab/Desktop/Anthony/UGVSM/3D_flow_consistent_UVCGANv2_vHE/scripts/20260221_Inverted_BIT2HE_kidney_tumor/20251124_Inverted_Combined_BIT2HE_tumor_kidney_all_Pretrain/model_m(autoencoder)_d(None)_g(vit-modnet)_pretrain-uvcgan2')
...
model_save_dir = os.path.join(
    ROOT_OUTDIR,
    f'{today_str}_BIT2HE_tumor_kidney_3DFlow_Multiscale_Content'
)
...
dataset_config = [
    {
        'dataset': {
            'name': 'cyclegan',  # just a label; it will be overridden in train() pipeline
            'domain': 'A',
...
### Debug the dataloader only when requested.
if cmdargs.debug_adjacent_pairs:
