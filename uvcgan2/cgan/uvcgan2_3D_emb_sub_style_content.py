...
class UVCGAN2_3D_emb_sub_style_content(ModelBase):
    ...
    def _setup_losses(self, config):
        ...
        # Multiscale content loss (sampled channels across encoder scales + ViT spatial).
        if self.is_train and getattr(self, "lambda_multiscale_content", 0) > 0:
            losses += [ 'multiscale_content' ]
    ...
    def __init__(
        ...
        # Scales where we compare content features:
        #   - enc1/enc2/enc3: encoder blocks (pre-downsample)
        #   - vit: spatial output of the ViT bottleneck
        default_scales = ("enc1", "enc2", "enc3", "vit")
        if multiscale_scales is None:
            self.multiscale_scales = default_scales
        else:
            if isinstance(multiscale_scales, str):
                raw_scales = [s.strip() for s in multiscale_scales.split(",")]
            else:
                raw_scales = list(multiscale_scales)
            filtered = tuple(s for s in raw_scales if s in default_scales)
            # Fall back to defaults if the provided list is empty or invalid.
            self.multiscale_scales = filtered or default_scales
        ...
        print(
            f"Initialized UVCGAN2_3D_emb_sub_style_content with "
        ...
        # L1 is robust and easy to interpret for feature consistency.
        self.criterion_multiscale_content = torch.nn.L1Loss()  # Loss for multiscale sampled-channel content consistency.
    ...
    def _ensure_multiscale_content_hooks(self, gen_ab, gen_ba):
        # Register hooks once to capture feature maps at multiple scales.
        # We keep the hooks alive for the model lifetime to avoid re-registering.
        if hasattr(self, "_ms_content_handles"):
            return

        self._ms_content_handles = {}
        self._ms_content_storage = {"ab": {}, "ba": {}}

        def make_hook(which, scale, take_vit=False):
            def hook(_module, _inputs, output):
                # Store the full feature map for the requested scale.
                # For ViT, output is (spatial, tokens) so we take output[0].
                feat = output[0] if take_vit else output
                self._ms_content_storage[which][scale] = feat
            return hook
        ...
        for which, gen in (("ab", gen_ab), ("ba", gen_ba)):
            # Encoder blocks (pre-downsample): conv.block output.
            block1 = get_block(gen, 0)
            block2 = get_block(gen, 1)
            block3 = get_block(gen, 2)

            if block1 is not None and hasattr(block1, "conv"):
                self._ms_content_handles[f"{which}_enc1"] = (
                    block1.conv.block.register_forward_hook(
                        make_hook(which, "enc1")
                    )
                )
            if block2 is not None and hasattr(block2, "conv"):
                self._ms_content_handles[f"{which}_enc2"] = (
                    block2.conv.block.register_forward_hook(
                        make_hook(which, "enc2")
                    )
                )
            if block3 is not None and hasattr(block3, "conv"):
                self._ms_content_handles[f"{which}_enc3"] = (
                    block3.conv.block.register_forward_hook(
                        make_hook(which, "enc3")
                    )
                )

            # ViT bottleneck spatial output (same H/W as its input).
            bottleneck = self._get_vit_bottleneck(gen)
            self._ms_content_handles[f"{which}_vit"] = (
                bottleneck.register_forward_hook(
                    make_hook(which, "vit", take_vit=True)
                )
            )

    def _clear_multiscale_content_storage(self):
        # Clear cached feature maps between forwards to avoid mixing steps.
        if not hasattr(self, "_ms_content_storage"):
            return
        self._ms_content_storage["ab"].clear()
        self._ms_content_storage["ba"].clear()

    def _save_single_feature_map(self, feat, save_path):
        """
        Save a single feature map (first sample, first channel) as a grayscale image.
        We normalize to [0, 1] for visualization.
        """
        if feat is None or feat.ndim != 4 or feat.shape[0] == 0:
            return
        fmap = feat[0, 0].detach()  # (H, W)
        fmap = fmap - fmap.min()
        denom = fmap.max()
        if denom.item() > 0:
    ...
    def backward_gen(self, direction):
        if direction == 'ab':
            ...
            # Multiscale content loss (sampled channels across encoder scales)
            if getattr(self, "lambda_multiscale_content", 0) > 0:
                ...
                if ms_loss is not None:
                    self.losses.multiscale_content = ms_loss
                    loss += float(self.lambda_multiscale_content) * self.losses.multiscale_content
